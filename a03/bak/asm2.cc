...
inline void processID(vector<Token> &curline, vector<Label*> &symbolTable,
	...
	if (size == 2) { // process the type that has one opcode and one register
		...
		int s = first.kind != ID ? first.toInt() : -1; // either ID, REGISTER, INT, HEXINT
		if (token.lexeme == "jr") {
			mask = 0x8 + (s << 21);
		} else if (token.lexeme == "jalr") {
			mask = 0x9 + (s << 21);
		} else if (token.kind == DOTWORD) {
			mask = s != -1 ? s : getFromTable(symbolTable, first.lexeme);
		}
	} else if (size == 6) {
...
inline bool check(vector<vector<Token> > &tokLines,
		vector<Label*> &symbolTable, vector<Label*> &cache, int &PC) {
	int result = 1;
	for (int i = 0, size; i < tokLines.size(); i++) {
		result *= checkLine(i, tokLines[i], symbolTable, cache, PC);
	}
	return result;
}
...
int main() {
	// symbol table here
	vector<Label *> symbolTable, cache;
	try {
		...
		int PC = 0;
		check(tokLines, symbolTable, cache, PC);
		for (int i = 0; i < cache.size(); i++) {
			if (!existInTable(symbolTable, cache[i]->name)) {
		...
		for (int line = 0; line < tokLines.size(); line++) {
			...
			if (token.kind == DOTWORD) {
				//	processDOTWORD(tokLines[line], symbolTable);
