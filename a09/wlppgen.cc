...
class Grammar {
	...
	struct TreeNode {
		...
		~TreeNode() {
/*
cout << dfa.symToStr(id) << " lexeme:" << lexeme << " RHS:";
	for (int i = 0; i < RHS.size(); i++){
cout << dfa.symToStr(RHS[i])<<" ";
	}cout << endl;
*/
			for (; !children.empty();){
	...
	void genSymbols(TreeNode *t) {
		...
		if (t->id == dcl) {
			...
			type = (t->children.front()->children.size() == 1) ? TYPE_INT
					: TYPE_INTSTAR;
			symbolTable.push_back(Pair<string, int> (lexeme, type));
		} else if (t->id == ID) {
	...
	list<Pair<string, int> > symbolTable;
	string buf, buf2;

public:
	Grammar(){ // initialise values for safety
		parseTree =NULL;
		numTerm = numNonTerm= numRules= numStates= numActions= start=startingRuleId = 0;
	}
...
const  string
				nonTerm[] = { "S", "dcl", "dcls", "expr", "factor", "lvalue",
						"procedure", "statement", "statements", "term", "test",
						"type" };
	...
	void printSymbolTable(ostream &out) {
		list<Pair<string, int> >::iterator it;
		for (it = symbolTable.begin(); it != symbolTable.end(); it++)
			out << it->x << " " << (it->y == TYPE_INT ? "int" : "int*")
					<< endl;
	}
...
*/
	...
	TreeNode *buildTree(istream &in){
		if (in.fail()) throw("Unexpected end of file."); //should not happen
		stringstream ss;
		int id, RHStemp, RHScount = 0;
		string line;
		vector <int> RHS;
		...
		TreeNode * t = new TreeNode(id, RHS);
		for (int i = 0, nextId, nextRule; i < RHScount; i++) {
			t->children.push_back(buildTree(in));
		}
		return t;
	}
...
ostream &operator<<(ostream &out, Grammar::TreeNode *t) {
	...
	int size = t->RHS.size();
	if (size== 0) {
		out << dfa.symToStr(t->id) << " " << t->lexeme;
	} else{
